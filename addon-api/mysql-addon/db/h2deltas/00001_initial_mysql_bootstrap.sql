...
CREATE INDEX IDX_PIPELINE_NAME ON pipelines(NAME(255));
CREATE INDEX IDX_PIPELINE_LABEL ON pipelines(LABEL);
CREATE INDEX IDX_PIPELINES_NAME_COUNTER ON pipelines(NAME(255), COUNTER);
...
CREATE INDEX IDX_STAGE_NAME ON STAGES(NAME(255));
CREATE INDEX IDX_STAGES_ORDERID ON STAGES(ORDERID);
CREATE INDEX IDX_STAGES_NAME_PIPELINEID ON STAGES(NAME(255), PIPELINEID);
...
CREATE TABLE STAGEARTIFACTCLEANUPPROHIBITED(
    id BIGINT NOT NULL PRIMARY KEY AUTO_INCREMENT,
...
CREATE INDEX IDX_STAGEARTIFACTCLEANUPPROHIBITED_PIPELINENAME ON STAGEARTIFACTCLEANUPPROHIBITED(PIPELINENAME(255));
...
CREATE INDEX IDX_BUILD_NAME ON BUILDS(NAME(255));
...
CREATE INDEX IDX_BUILDS_AGENTID_STAGEID_NAME ON BUILDS(NAME(255), AGENTUUID, STAGEID, STATE, RESULT);
CREATE INDEX IDX_BUILDS_NAME_STATE_STAGEID ON BUILDS(NAME(255), STATE, STAGEID);
CREATE INDEX IDX_BUILDS_NAME_STAGE_ID ON BUILDS(NAME(255), STAGEID);
CREATE INDEX IDX_BUILDS_STAGEID ON BUILDS(STAGEID);


CREATE VIEW _BUILDS(ID, NAME, STATE, RESULT, AGENTUUID, SCHEDULEDDATE, STAGEID, IGNORED, RUNONALLAGENTS, ORIGINALJOBID, RERUN, RUNMULTIPLEINSTANCE, PIPELINEID, PIPELINENAME, PIPELINELABEL, PIPELINECOUNTER, STAGENAME, STAGECOUNTER, FETCHMATERIALS, CLEANWORKINGDIR, RERUNOFCOUNTER) AS
SELECT
    B.ID,
    B.NAME,
    B.STATE,
...
CREATE INDEX IDX_MOD_NEW_MATERIAL_ID ON MODIFICATIONS(MATERIALID);
CREATE INDEX IDX_MODIFICATIONS_MATERIALID_ID ON MODIFICATIONS(MATERIALID, ID);
CREATE INDEX IDX_MODIFICATIONS_REVISION ON MODIFICATIONS(REVISION(255));
...
CREATE INDEX IDX_ENV_JOB_ID ON ENVIRONMENTVARIABLES(ENTITYID);
